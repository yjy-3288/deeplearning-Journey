...
在纠正协变量偏移时，我们需要计算样本的**重要性权重**：$\beta = \frac{p(x)}{q(x)}$
（其中 $p(x)$ 为测试集密度，$q(x)$ 为训练集密度）

- 现实困境:
直接计算高维复杂数据（如图像特征）的概率密度 $p(x)$ 和 $q(x)$，在数学和计算上几乎是不可能完成的任务
...
$$\beta = \frac{p(x)}{q(x)} = \frac{P(z=1|x)}{P(z=-1|x)}$$

总结：
> 因为算不出两个数据分布的具体形状，我们就用 $z$ 标签强行造出一个新任务，训练一个分类器去玩“猜猜我来自哪个集合”的游戏。靠猜出的概率比值，直接套算出我们要的样本权重。
